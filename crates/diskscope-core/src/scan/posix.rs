...
//!
//! The slow path, kept for two reasons: filesystems that reject
//! `getattrlistbulk` (network shares, some FUSE volumes), and as the reference
//! implementation the bulk reader is tested against — `tests/scan.rs` asserts
...
pub fn read_dir(fd: c_int, mut push: impl FnMut(Ent<'_>)) -> io::Result<()> {
    ...
    loop {
        ...
        // SAFETY: `d_name` holds `d_namlen` bytes plus a NUL.
        let name = unsafe { std::slice::from_raw_parts(d.d_name.as_ptr() as *const u8, name_len) };
        if name == b"." || name == b".." {
            continue;
        }
